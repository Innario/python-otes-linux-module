...
Request server with
> curl -v http://localhost:9998/?status=400 --http0.9
"""
...
def http_response(status=200, body=""):
    ...
    return f"""
HTTP/1.0 {http_status.value} {http_status.phrase}
Server: otus-student
    ...
    while True:
        ...
        with client:
            ...
            while True:
                ...
                if not data:
                    break
                headers_list = data.decode().splitlines()
                ...
                """
                print(report)
                response = http_response(status=int(params_dict.get("status", 200)), body=report)
